...
class Privilege {
  ...
  static Future<PrivilegeStatus> camera() async {
    ...
    value = statuses[Permission.camera] == PermissionStatus.granted && statuses[Permission.microphone] == PermissionStatus.granted;
    if (GetPlatform.isIOS) {
      value = statuses[Permission.photos] == PermissionStatus.granted;
    }
    if (GetPlatform.isAndroid) {
  ...
  static Future<bool> storage() async {
    ...
    if (GetPlatform.isAndroid) {
      ...
      text = 'Please allow access to storage space, settings will be opened soon';
    }
